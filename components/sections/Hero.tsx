...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-anset-cream">
      <LeafPattern color="var(--anset-blue)" opacity={0.05} size={200} className="hidden lg:block -left-12 top-20" />

      <div className="container-anset relative z-10 py-14 md:py-20 lg:py-24 grid grid-cols-1 lg:grid-cols-[1fr_1.05fr] gap-10 lg:gap-14 items-center">

        <div className="flex flex-col justify-center order-2 lg:order-1">
          <span className="inline-flex items-center gap-2 text-anset-corail-dark text-[11px] font-black uppercase tracking-[2.5px] mb-5">
            <span className="w-1.5 h-1.5 rounded-full bg-anset-corail shadow-[0_0_0_4px_rgba(211,111,107,0.18)]" aria-hidden="true"></span>
            Lancement métropole
          </span>

          <h1 className="text-4xl md:text-5xl lg:text-[46px] font-black text-anset-blue leading-[0.96] tracking-[-0.045em]">
            À vos côtés,<br />
            à tout moment.<br />
            <span className="text-anset-corail">Même en France.</span>
          </h1>

          <p className="mt-5 text-sm md:text-base text-anset-slate leading-relaxed font-medium max-w-lg">
          ...
          <div className="mt-8 pt-6 border-t border-anset-blue/10 flex flex-wrap items-center gap-x-6 gap-y-3">
            <div className="flex items-center gap-2.5">
              <div className="bg-trustpilot text-white w-[22px] h-[22px] rounded flex items-center justify-center text-sm font-black leading-none">★</div>
              <div className="text-[11px] text-anset-slate leading-tight">
                <span className="text-trustpilot text-[11px] tracking-wider block leading-none mb-0.5">★★★★★</span>
                <strong className="text-anset-blue font-black">{TRUSTPILOT.rating}/5</strong> sur {TRUSTPILOT.name}
              </div>
            </div>
            <div className="flex items-center gap-2.5">
              <CalendarHeart className="w-5 h-5 text-anset-menthe" strokeWidth={1.8} aria-hidden="true" />
              <div className="text-[11px] text-anset-slate leading-tight">
