...
const inputCls = "w-full px-3.5 py-2.5 text-sm rounded-xl border border-anset-blue/15 bg-white focus:outline-none focus:border-anset-corail focus:ring-2 focus:ring-anset-corail/15 transition-colors font-medium text-anset-blue placeholder:text-anset-slate/50";
  ...
  if (success) {
    return (
      <div className="bg-white rounded-2xl border-2 border-anset-menthe/30 p-8 md:p-10 text-center shadow-premium-sm">
        ...
        <button onClick={() => setSuccess(false)} className="inline-flex items-center gap-2 text-sm font-black text-anset-blue hover:text-anset-corail transition-colors">
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-7">
      ...
      <div>
        ...
        <div
          ...
          className="border-2 border-dashed border-anset-blue/20 rounded-2xl p-7 text-center cursor-pointer hover:border-anset-corail hover:bg-anset-corail-soft/40 transition-all"
        >
          ...
          <div className="mt-3 space-y-2">
            {files.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 bg-anset-mist/40 rounded-xl px-3.5 py-2.5 border border-anset-blue/8">
                ...
                <button type="button" onClick={() => removeFile(entry.id)} className="w-7 h-7 rounded-full bg-white hover:bg-anset-corail-soft text-anset-slate hover:text-anset-corail-dark transition-colors flex items-center justify-center flex-shrink-0" aria-label="Retirer">
      ...
      <div className="flex items-start gap-2.5 pt-2">
        <input type="checkbox" id="consent" name="consent" required className="mt-1 w-4 h-4 rounded border-anset-blue/30 text-anset-corail focus:ring-anset-corail" />
        <label htmlFor="consent" className="text-xs text-anset-slate font-medium leading-relaxed">
          J'accepte que mes données et les pièces transmises soient communiquées à Groupama Protection Juridique pour le traitement de ma demande de remboursement, conformément à la <a href="/politique-de-confidentialite" className="text-anset-corail-dark underline hover:text-anset-corail font-bold">politique de confidentialité</a>. *
        </label>
      </div>
